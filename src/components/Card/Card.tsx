...
const getSuitPattern = (suit: CardType["suit"], rank: CardType["rank"]) => {
  ...
  const suitElements: JSX.Element[] = [];
      ...
      >
        ...
        <div className="absolute inset-1 border border-yellow-400 rounded-md opacity-60">
          <div className="w-full h-full relative overflow-hidden">
            ...
            <svg className="absolute inset-0 w-full h-full opacity-30" viewBox="0 0 100 100">
              <defs>
                <pattern id="diamonds" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
                  <polygon points="10,5 15,10 10,15 5,10" fill="#fbbf24" opacity="0.3" />
                </pattern>
              </defs>
            ...
            {/* Center logo */}
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-yellow-400 font-bold text-xs opacity-80">♠♥♦♣</div>
            </div>
          </div>
        </div>
      </motion.div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-1 bg-white rounded-md p-1 flex flex-col justify-between">
        {/* Top left corner */}
        <div className={`flex flex-col items-start ${isRed ? "text-red-600" : "text-gray-900"}`}>
          <span className="font-bold leading-none text-sm">{card.rank}</span>
          <SuitIcon suit={card.suit} className="w-3 h-3 mt-0.5" filled />
        </div>

        {/* Center suit pattern */}
        <div className={`absolute inset-0 ${isRed ? "text-red-600" : "text-gray-900"}`}>
          {getSuitPattern(card.suit, card.rank)}
        </div>

        {/* Bottom right corner (rotated) */}
        <div className={`flex flex-col items-end rotate-180 ${isRed ? "text-red-600" : "text-gray-900"}`}>
          <span className="font-bold leading-none text-sm">{card.rank}</span>
          <SuitIcon suit={card.suit} className="w-3 h-3 mt-0.5" filled />
        </div>
        ...
        {(card.rank === "J" || card.rank === "Q" || card.rank === "K") && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className={`text-xs font-semibold opacity-80 ${isRed ? "text-red-600" : "text-gray-900"}`}>
              {card.rank === "J" && "JACK"}
              {card.rank === "Q" && "QUEEN"}
              {card.rank === "K" && "KING"}
            </div>
